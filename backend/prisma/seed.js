...
const main = async () => {
  try {
    // Load JSON files
    const seedDevices = JSON.parse(await fs.readFile('./prisma/seedDevices.json', 'utf8'));
    const seedSensorData = JSON.parse(await fs.readFile('./prisma/seedSensorData.json', 'utf8'));
    const seedBlocks = JSON.parse(await fs.readFile('./prisma/seedBlocks.json', 'utf8'));
